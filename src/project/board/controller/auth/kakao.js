...
const login = (req, res, next) => {
    /* kakao 로부터 받은 인증코드가 없다. */
    passport.authenticate('kakao')(req, res, next);
    /* client 에게 kakao 에 대한 redirect 를 유도한다. */
};
...
const callback = (req, res, next) => {
    /* kakao 로부터 받은 인증코드가 있다. */
    /* 인증코드가 있으면 kakao 에 accessToken 및 사용자 정보를 요청한다. */
    passport.authenticate('kakao')(req, res, next);
    /* 네트워크 요청이므로 async 로 실행된다. */
    /* 응답이 오면 kakao 전략이 실행된다. */
};
